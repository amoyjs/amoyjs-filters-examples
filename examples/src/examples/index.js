import { format } from 'path';

// Order here is the dat-gui order
export {default as PageCurlExample} from './page-curl';
export {default as GameboyExample} from './gameboy';
export {default as SnowExample} from './snow';
export {default as RainExample} from './rain';
export {default as LightSweepExample} from './light-sweep';
export {default as LensHaloExample} from './lens-halo';
export {default as CloudExample} from './cloud';
export {default as InnerOutlineExample} from './inner-outline';
export {default as Light2DExample} from './light2d';
export {default as FluidExample} from './fluid'
export {default as SparkExample} from './spark'
export {default as PixelVibrationExample} from './pixel-vibration'
export {default as ReflectionExample} from './reflection'
export {default as WaterReflectionExample} from './water-reflection'
export {default as BloodSplashExample} from './blood-splash'
export {default as CrossProcessingExample} from './cross-processing'
export {default as MagnifyExample} from './magnify'
export {default as FishEyeExample} from './fisheye'
export {default as BarrelDistortionExample} from './barrel-distortion'
export {default as ClearBackgroundExample} from './clear-color'
export {default as WarholPhotoExample} from './warhol'
export {default as ColorblindExample} from './colorblind'